    ...
    @menus = [
      ...
      { :label => "#{ApplicationHelper::DM_AI_ICON}優先(アクセス日順)",
        :path => twitters_path(
          ...
          grp_sort_spec: "{am}m|{r}({restrict})::{az}",
      ...
      { :label => "#{ApplicationHelper::DM_AI_ICON}優先tmp",
        :path => twitters_path(
          ...
          #force_disp_day: 10,
          grp_sort_by: TwittersController::GRP_SORT::GRP_SORT_SPEC,
          grp_sort_spec: "{az}::評価{r}({restrict})",
      ...
      { :label => "-", :path => "" },#------------------
      { :label => "#{ApplicationHelper::DM_AI_ICON}ファイル少ない順",
        :path => twitters_path(
          #page_title: "all",
      ...
      { :label => "#{ApplicationHelper::DM_AI_ICON}評価/登録日順",
        :path => twitters_path(
          ...
          grp_sort_by: TwittersController::GRP_SORT::GRP_SORT_SPEC,
          grp_sort_spec:  "{am}|{r}({restrict})::{az}|{p15}",
          rating: 85,
          hide_within_days: 14,
          #pfilenum: -1500,
          select_max: 22,
...
=end
      ...
      { :label => "最新ファイル 全",
          :path => artists_twt_index_path(
            ...
            grp_sort_spec: "{az}|{r}#{Twitter::TWT_H_SEPARATOR}{p15}",
          )
      },
